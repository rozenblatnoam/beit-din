...
לכל תיק, הדיין יכול ליצור שני מסמכים מובנים:
- **פרוטוקול דיון** (`hearing_protocol`) — תיעוד מהלך הדיון, טענות הצדדים
- **פסק דין** (`verdict`) — ההכרעה הסופית

**אם Google Drive מוגדר:** לחיצה על "צור" יוצרת **Google Doc** בתיקיית התיק (חוויה דמוית Word — עיצוב עשיר, שמירה אוטומטית של Google, שיתוף, ייצוא ל-`.docx` דרך File → Download). המסמך נפתח בכרטיסייה חדשה לעריכה, ובדף DinLink נשארת תצוגה מקדימה.

**אם Drive לא מוגדר:** המערכת חוזרת אוטומטית ל-textarea פנימי עם auto-save (1.5 שניות אחרי הקלדה אחרונה).

הלקוח ועו"ד מקבלים התראה כשנוצר מסמך כזה.
...
| `case_protocols` | `case_id`, `author_dayan_id` | פרוטוקול ופסק דין — `type` (`hearing_protocol`/`verdict`), `content` (גיבוי טקסטואלי), `drive_file_id` + `drive_edit_url` (כשגוגל דוקס פעיל) |

...
import { Briefcase, ShieldCheck, Eye, EyeOff, AlertCircle } from 'lucide-react';
import { useApp } from '../context/AppContext';
import styles from './DayanLogin.module.css';

export default function LawyerLogin() {
  const { loginLawyer, googleLawyerLoginUrl } = useApp();
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await loginLawyer(email, password);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.card}>
        <div className={styles.header}>
          ...
          <h1 className={styles.title}>כניסת עו"ד / טו"ר</h1>
        ...
        <div className={styles.form}>
          ...
          <button className={styles.submitBtn} onClick={handleSubmit} disabled={loading}>
            {loading ? <span className={styles.spinner} /> : (
              <><Briefcase size={15} /> כניסה למערכת</>
            )}
          </button>

          <div className={styles.divider}><span>או</span></div>

          <a href={googleLawyerLoginUrl()} className={styles.googleBtn}>
        ...
        <div className={styles.hint}>
          <ShieldCheck size={13} />
          <span>עורכי דין וטוענים רבניים נרשמים על ידי מנהל המערכת בלבד</span>
        </div>
      </div>
    </div>
  );
